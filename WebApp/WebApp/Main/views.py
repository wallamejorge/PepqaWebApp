...
def AdminDashboard(request):
	...
	fp.close()	
	# Redenrizar la plantilla con las variables
	html=page.render(Context({'usersList':User.objects.all(),'groupsList':Group.objects.all(),'offersList':Offer.objects.all(),'friendsList':Friendship.objects.all()}))
	return HttpResponse(html)
#----------------------------------------------------------------------------- #
...
def AdminNewFriendship(request):
	try :
		...
		FriendshipFrom =Frienship.objects.create(friendship_from=UserFrom,friendship_to=ToFrom)
		FriendshipFrom.save()
		return HttpResponseRedirect('/Dash/')
	except:
		fp=open(PATH_SITE+PATH_PAGES+'PopUp.html')
