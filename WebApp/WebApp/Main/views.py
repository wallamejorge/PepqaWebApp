...
def authentication(request):
	if 'username' in request.GET and request.GET['username']:
		...
		if (userpass == usrpass) and (username == usrname) :
			...
			userlist = getFriendsList(username);
			foto = "http://i.imgur.com/AYORqYs.png" ;
...
def getFriendsList(usremail):
	li = Friendship.objects.filter(user1=usremail)
	elements = []
	for i in li:
    		elements.append(i.user2)
	return elements
