...
# List's Data :
	...
	# FriendsRequestList
	myfriendsrequestlist = getMyFriendsList(myemail);
	# GroupsList
...
def getMyGroupsList(email):
	MyGroups = GroupMembership.objects.filter(groupmembership_user=email)
	groups= []
	for fi in MyGroups:
    		groups.append(fi.groupmembership_name)
	return groups
...
def getMyArticleAuthorsList(email):
	MyArticles = ArticleAuthor.objects.filter(articleauthors_articleauthor=email)
	articles= []
